...
//		alert(liurl);	// 작동하는지 확인용
		// (2)  class='notebook' 지정한 article의 요소에 ajax로 파일을 받아서 지정
		$.ajax({
			type : 'get',			// 보낼 값이 없으니 get?	
			url : liurl,			// 클릭한 곳의 data-url로 보낸다?
			dataType : 'html',		// html 형식으로 보낸다.
			success : function(data){	// 성공하면 .notebook에 html을 data로 바꾼다.
				$('.notebook').html(data);
			}
		})
