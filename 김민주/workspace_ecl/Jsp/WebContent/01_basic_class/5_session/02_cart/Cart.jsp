...
//	각 제품의 정보를 getParameter로 받아온다. (form 으로 보내온 값들을 getParameter 로 받아온다.)
//  보내온 정보들은 각 제품.jsp 파일에서 form 태그로 보내온다.
	id = request.getParameter("id");	// 제품의 id
	name =request.getParameter("name");	// 제품의 이름
	price = price + Integer.parseInt(request.getParameter("price")); 
	// 2. 세션의 cart 속성을 얻어온다. 
	Object list = session.getAttribute("cart"); // null 값이 들어있다. 셋팅한적이 없어서.
...
// 	glist.add(g);
// 	위의 변수를 한번만 쓸거같으면 굳이 변수 선언 안하고 한줄로 축약 가능
	glist.add(new Goods(id,name,price));
	
	// 5. 세션에 cart 라는 이름에 ArrayList를 저장
	// cart 라는 키값에 glist를 벨류로 넣어준다?
